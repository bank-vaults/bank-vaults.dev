...
1. In the `vault.banzaicloud.com` custom resource (for example, in this [sample CR](https://github.com/bank-vaults/vault-operator/blob/v{{< param "latest_vault_operator" >}}/deploy/examples/cr.yaml)) of `cluster1`, define an `externalConfig` section. Fill the values of the `kubernetes_ca_cert`, `kubernetes_host`, and `token_reviewer_jwt` using the data collected in the previous steps.
