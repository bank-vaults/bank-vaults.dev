...
Both the operator and the chart generate a Kubernetes Secret holding the TLS certificates, this is named `${VAULT_CR_NAME}-tls`. For most examples in the [vault-operator repository](https://github.com/bank-vaults/vault-operator/tree/main/deploy/examples), the name of the secret is `vault-tls`.
